...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(VGui_Paint,
		         ...
		         "5377866",
		         "55 8B EC E8 ?? ?? ?? ?? 8B 10 8B C8 5D FF 62 ?? 55 8B EC 83 EC 08 8B 0D 44",
		         "BMS-Retail",
		         "55 8B EC E8 ?? ?? ?? ?? 8B 10 8B C8 5D FF 62 ?? 8B 0D 1C");
		PATTERNS(
	...
	namespace client
	{
		...
		PATTERNS(
		    ...
		    "5377866",
		    "55 8B EC 81 EC FC 01 00 00 53 56 57 8B F9 89 7D FC 8D 8F 5C 03 00 00 E8",
		    "te120",
		    "55 8B EC 81 EC F0 01 00 00 A1 ?? ?? ?? ?? 33 C5 89 45 ?? 53 8B 5D",
		    "icemod",
		    "55 8B EC 81 EC D8 01 00 00 A1 ?? ?? ?? ?? 33 C5 89 45 FC 53 8B 5D 08 57 8B F9 89 9D 3C FE FF FF 89 BD 50 FE FF FF 8D 8F 5C 03 00 00 E8");
		PATTERNS(
		    CViewRender__Render,
		    "5135",
		    "81 EC 98 00 00 00 53 56 57 6A 04 6A 00 68 ?? ?? ?? ?? 6A 00 8B F1 8B ?? ?? ?? ?? ?? 68 ?? ?? ?? ?? FF ?? ?? ?? ?? ?? 8B BC 24 A8 00 00 00 8B 4F 04",
		    "5377866",
		    "55 8B EC 81 EC 30 01 00 00 53 56 57 8B F9 33 DB 8B 0D",
		    "te120",
		    "55 8B EC 81 EC 08 01 00 00 A1 ?? ?? ?? ?? 33 C5 89 45 ?? 56 8B 75",
		    "icemod",
		    "55 8B EC 81 EC 10 01 00 00 A1 ?? ?? ?? ?? 33 C5 89 45 ?? 53 8B 5D");
		PATTERNS(
		    GetClientModeNormal,
		    "5135",
		    "B8 ?? ?? ?? ?? 84 05 40 31 43 10 75 ?? 09 05 40 31 43 10 B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 68 D0 3A 2C 10",
		    "5377866",
		    "A1 ?? ?? ?? ?? A8 01 75 ?? 83 C8 01 B9 ?? ?? ?? ?? A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? 68 00 02 00 00");
		PATTERNS(
		    UTIL_TraceLine,
		    "5135",
	...
	namespace vguimatsurface
	{
		PATTERNS(
		    ...
		    "5377866",
		    "55 8B EC 64 A1 ?? ?? ?? ?? 6A FF 68 ?? ?? ?? ?? 50 64 89 25 ?? ?? ?? ?? 83 EC 14 80 3D ?? ?? ?? ?? 00",
		    "BMS-Retail",
