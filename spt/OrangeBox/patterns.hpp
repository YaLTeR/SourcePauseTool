...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(
			...
			"1910503",
			"56 8B F1 8B 06 8B 50 10 FF D2 84 C0 74 ?? 8D 4E 04 E8 ?? ?? ?? ?? 8B 86 ?? ?? ?? ?? C6 86 ?? ?? ?? ?? 00 85 C0");
		PATTERNS(
			MiddleOfLoadSaveGame, // this is after the check has been done to see if the save file exists
			"5135",
			"6A 00 68 ?? ?? ?? ?? B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 ?? ?? ?? ?? ?? 02 7C ?? 80 ?? ?? ?? ?? ?? 00 74");
		PATTERNS(
			MiddleOfState_LoadGame, // this is if loading the game failed and the host is shutting down
			"5135",
			"E8 ?? ?? ?? ?? 80 BE 24 03 00 00 00 74 14 8B ?? ?? ?? ?? ?? 8B 11 8B 42 ?? FF ?? C6 ?? ?? ?? ?? ?? ?? C7 46 04 04 00 00 00 C7 06 04 00 00 00 5E C3");
	} // namespace engine
