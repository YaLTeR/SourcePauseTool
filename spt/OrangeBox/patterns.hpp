...
namespace patterns
{
	namespace engine
	{
		...
		PATTERNS(
		    Record_Func,
		    "5135",
		    "81 EC 08 02 00 00 56 8B F1 80 BE 46 06 00 00 00 0F 84 ?? ?? ?? ?? 53 8D 5E 04");
		PATTERNS(WriteConsoleCommand,
		         "5135",
		         "53 8B 5C 24 ?? 85 DB 57 8B F9 74 ?? 80 3B 00 74 ?? 83 BF ?? ?? ?? ?? 00 74 ??");
		...
		PATTERNS(
		    ...
		    "A3 ?? ?? ?? ?? B9 ?? ?? ?? ?? A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? D9 1D ?? ?? ?? ?? B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? D9 1D",
			"portal 2",
			"89 96 C4 00 00 00 8B 86 C8 00 00 00 8B CE A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? D9 1D ?? ?? ?? ?? 8B CE A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? D9 1D ?? ?? ?? ?? 8B CE E8 ?? ?? ?? ?? D9 1D",
			"source 2009",
			"89 8F C4 00 00 00 8B 97 C8 00 00 00 8B CF 89 15 ?? ?? ?? ?? E8 ?? ?? ?? ?? D9 1D ?? ?? ?? ?? 8B CF E8 ?? ?? ?? ?? D9 1D");
		PATTERNS(
		    StopRecording,
		    "5135",
		    "56 8B F1 8B 06 8B 50 ?? FF D2 84 C0 74 ?? 8B 86 ?? ?? ?? ?? 85 C0 57 8D BE ?? ?? ?? ?? 74 ??");
		PATTERNS(
			ServerState,
		    "1",
		    "83 F8 01 0F 8C ?? ?? 00 00 3D 00 02 00 00 0F 8F ?? ?? 00 00 83 3d ?? ?? ?? ?? 02 7D");
	} // namespace engine
