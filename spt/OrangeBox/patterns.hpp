...
namespace patterns
{
	...
	namespace client
	{
		...
		PATTERNS(
		    ...
		    "BMS-Retail",
		    "8b 91 58 02 00 00 83 ?? ff 74 ?? a1 ?? ?? ?? ?? 8b ca 81 e1 ff 1f 00 00 c1 ea ?? 03 c9");
		PATTERNS(
		...
		PATTERNS(
		    ...
		    "BMS-Retail",
		    "55 8B EC 81 EC 88 01 00 00 A1 ?? ?? ?? ?? 33 C5 89 45 ?? 53 8B 5D ?? 56 57 8B F9 89 9D ?? ?? ?? ?? 8B 0D");
		...
		PATTERNS(
			MiddleOfCViewRenderRender,
			"BMS-Retail",
		    "8B 0D ?? ?? ?? ?? C6 05 ?? ?? ?? ?? 01 6A 02 8B 01 FF 90 ?? ?? 00 00", 
			"5135",
			"8B 0D ?? ?? ?? ?? C6 05 ?? ?? ?? ?? 01 8B 01 8B 90 ?? ?? 00 00 6A 02");
		PATTERNS(ResetToneMapping,
		    "5135",
	...
	namespace server
	{
		PATTERNS(
			...
			"hdtf",
			"55 8B EC 83 EC 30 56 8B F1 8B 8E ?? ?? ?? ?? 80 B9 ?? ?? 00 00 00 74 ?? 8B 46 ?? 83 48 ?? 02 32 C0 5E 8B E5 5D C3 E8 ?? ?? ?? ?? 84 C0 75 ?? 8B 4E ?? E8 ?? ?? ?? ?? D9 55 ??");
	...
	namespace vphysics
	{
		PATTERNS(
		    ...
		    "5135",
		    "16 05 ?? ?? ?? ?? 01 83 EE 01 3B 74 24 28 7D D3 8B 4C 24 38",
		    "1910503",
		    "16 05 ?? ?? ?? ?? 01 4E 3B 75 F0 7D D3 8B 8D DC FD FF FF");
		PATTERNS(
			GetShadowVelocity, 
			"5135", 
			"8B 41 ?? 8B 50 ?? 8B 82 ?? ?? ?? ?? 83 EC 20",
		    "BMS-Retail",
		    "55 8B EC 83 EC ?? A1 ?? ?? ?? ?? 33 C5 89 45 ?? 8B 41 ?? 56 8B 75 ?? 8B 40 ?? 8B 80 ?? ?? ?? ?? 85 F6");
		PATTERNS(GetPosition,
		         "5135",
		         "8B 49 ?? 81 EC 80 00 00 00 8D 04 24 50 E8 ?? ?? ?? ?? 8B 84 24 ?? ?? ?? ??");
		PATTERNS(
			GetShadowPosition, 
			"5135", 
