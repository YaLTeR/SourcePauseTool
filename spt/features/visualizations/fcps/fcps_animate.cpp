...
void FcpsAnimator::Draw(MeshRendererDelegate& mr)
{
	...
	// draw current step *AND* any steps that we increment over

	while (IsDrawing())
	{
		DrawCurrentStep(mr);
		using per = anim_clock::duration::period;
		if (curStepDur.count() * per::num * (double)newStepHz < per::den)
			break;

		stepIdx++;
		curStepDur -=
		    std::chrono::duration_cast<anim_clock::duration>(std::chrono::duration<float>{1 / newStepHz});
	};
}
