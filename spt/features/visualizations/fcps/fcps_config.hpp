...
/*
* Replicates the game's FCPS code and populates the event fields except for the caller. Assumes
* that the event lists are empty.
*/
void FcpsImpl(FcpsEvent& event, IServerEntity* pEntity, const Vector& vIndecisivePush, unsigned int fMask);

using fcps_event_id = uint64_t;
constexpr fcps_event_id FCPS_INVALID_EVENT_ID = 0;
#define FCPS_PR_ID PRIu64

// inclusive [from, to]
using fcps_event_range = std::pair<fcps_event_id, fcps_event_id>;
constexpr fcps_event_range FCPS_INVALID_EVENT_RANGE{FCPS_INVALID_EVENT_ID, FCPS_INVALID_EVENT_ID};

inline bool FcpsIdInRange(fcps_event_id id, fcps_event_range r)
{
	if (r.first > r.second)
		return false;
	if (r.first == FCPS_INVALID_EVENT_ID || r.second == FCPS_INVALID_EVENT_ID)
		return false;
