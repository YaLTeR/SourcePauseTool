		...
		case TYPE_UINT64:
			{
				...
				memcpy( m_sValue, src.m_sValue, sizeof(uint64) );
			}
			break;
		case TYPE_COLOR:
			{
...
bool EvaluateConditional( const char *str )
{
	...
	if ( bXboxUI )
	{
		bResult = !stricmp( "[$X360]", str );
	}
	else
	{
		bResult = !stricmp( "[$WIN32]", str );
	}
...
bool KeyValues::LoadFromBuffer( char const *resourceName, CUtlBuffer &buf, IBaseFileSystem* pFileSystem, const char *pPathID )
{
	...
	do 
	{
		...
		if ( !stricmp( s, "#include" ) )	// special include macro (not a key name)
		{
...
void KeyValues::RecursiveLoadFromBuffer( char const *resourceName, CUtlBuffer &buf )
{
	...
	while ( 1 )
	{
		...
		else 
		{
			...
			const char *peek = ReadToken( buf, wasQuoted );
			if ( wasConditional )
			{
