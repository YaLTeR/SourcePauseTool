...
void ConCommandBase::SetNext(ConCommandBase* pCmd) {
	m_pNext = pCmd;
}
...
ConCommand::ConCommand( const char *pName, FnCommandCallback callback, const char *pHelpString /*= 0*/, int flags /*= 0*/, FnCommandCompletionCallback completionFunc /*= 0*/ )
{
	// Set the callback
...
ConVar::ConVar( const char *pName, const char *pDefaultValue, int flags, const char *pHelpString, FnChangeCallback callback )
{
	Create( pName, pDefaultValue, flags, pHelpString, false, 0.0, false, 0.0, callback );
}

ConVar::ConVar( const char *pName, const char *pDefaultValue, int flags, const char *pHelpString, bool bMin, float fMin, bool bMax, float fMax, FnChangeCallback callback )
{
...
void ConVar::InstallChangeCallback( FnChangeCallback callback )
{
	...
	if ( m_pParent->m_fnChangeCallback )
	{
		...
		m_pParent->m_fnChangeCallback( this, m_pszString );
	}
}
...
void ConVar::ChangeStringValue( const char *tempVal)
{
	...
	g_pCVar->CallGlobalChangeCallback( this, pszOldValue );
